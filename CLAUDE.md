...
- **Mail delivery is push, not polled**: mbsync has no IMAP IDLE support, so `goimapnotify` (`mail/goimapnotify.yaml`) holds the idle connection and runs `mail/on-new-mail.sh` → `mbsync holten` + `notmuch new` + `notify-send`. It watches **INBOX only** — that is just the trigger; the handler syncs every folder, since scope is set by `Patterns` in `mbsyncrc`. Watching all folders would open one IDLE connection each (~18) and get refused. `mail/mbsync.timer` re-runs the same handler every 30 min as a backstop for a silently dropped IDLE connection. These are the repo's **only checked-in systemd units** (`~/.config/systemd/user/`, enabled by `install.sh`); `Restart=always` on the daemon exists because `passwordCMD` needs the Yubikey, which usually isn't unlocked yet at session start.
